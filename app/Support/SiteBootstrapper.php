...
class SiteBootstrapper
{
    ...
    private function bootstrapContentTypes(Site $site): void
    {
        ...
        foreach ($this->defaultContentTypes($site) as $type) {
    ...
    private function bootstrapTaxonomies(Site $site): void
    {
        ...
        foreach ($this->defaultTaxonomies($site) as $taxonomy) {
    ...
    private function bootstrapBackendMenus(Site $site): void
    {
        ...
        foreach ($this->defaultBackendChildMenus($site) as $parentModule => $children) {
    ...
    private function defaultContentTypes(Site $site): array
    {
        $types = [
            ['code' => 'home', 'name' => '首頁'],
        ...
        if ($this->moduleEnabled($site, 'news')) {
            $types[] = ['code' => 'news', 'name' => '最新消息'];
        }

        return $types;
    }

    private function defaultTaxonomies(Site $site): array
    {
        $taxonomies = [];

        if ($this->moduleEnabled($site, 'news')) {
            $taxonomies[] = ['code' => 'newsCate', 'name' => '最新消息分類', 'is_hierarchical' => false];
            $taxonomies[] = ['code' => 'newsTag', 'name' => '最新消息標籤', 'is_hierarchical' => false];
        }

        if ($this->moduleEnabled($site, 'products')) {
            $taxonomies[] = ['code' => 'productCate', 'name' => '產品分類', 'is_hierarchical' => true];
            $taxonomies[] = ['code' => 'productTag', 'name' => '產品標籤', 'is_hierarchical' => false];
        }
    ...
    private function defaultBackendMenus(Site $site): array
    {
        ...
        if ($this->moduleEnabled($site, 'news')) {
            $menus[] = ['title' => '最新消息', 'module_key' => 'news', 'route_name' => 'admin.news.index', 'icon' => 'bx bx-file'];
        }

        if ($this->moduleEnabled($site, 'products')) {
            $menus[] = ['title' => '產品', 'module_key' => 'products', 'route_name' => 'admin.products.index', 'icon' => 'bx bx-file'];
        }

        if ($this->moduleEnabled($site, 'contact')) {
            $menus[] = ['title' => '聯絡我們', 'module_key' => 'contact', 'route_name' => 'admin.contact.index', 'icon' => 'bx bx-detail'];
        }
    ...
    private function defaultBackendChildMenus(Site $site): array
    {
        $children = [
            'home' => [
    ...
    private function moduleEnabled(Site $site, string $module): bool
    {
        $enabled = $site->settings['enabled_modules'] ?? SiteModuleManager::standardModuleKeys();
