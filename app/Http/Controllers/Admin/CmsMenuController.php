...
class CmsMenuController extends Controller
{
    ...
    public function index(AdminContext $context, Request $request): View
    {
        ...
        $parent = $parentId
            ? CmsMenu::query()->where('site_id', $site->id)->with('parent')->findOrFail($parentId)
            : null;
        ...
        return view('admin.menus.index', [
            ...
            'menus' => $menus,
            'trash' => $trash,
            'trashCount' => CmsMenu::query()->where('site_id', $site->id)->where('location', $location)->onlyTrashed()->count(),
    ...
    public function create(AdminContext $context, Request $request): View
    {
        ...
        return view('admin.menus.form', [
            ...
            'menu' => null,
            'location' => $location,
            'parentOptions' => $this->parentOptions($site->id, $location),
            'values' => [
                'location' => $location,
                'parent_id' => $parentId,
                'type' => 'custom',
    ...
    public function store(AdminContext $context, Request $request): RedirectResponse
    {
        ...
        CmsMenu::query()->create([
            ...
            'sort_order' => $this->nextSortOrder($site->id, $data['location'], $parentId),
        ]);

        return redirect()
            ->route('admin.menus.index', array_filter(['location' => $data['location'], 'parent_id' => $parentId]))
    ...
    public function edit(AdminContext $context, CmsMenu $menu): View
    {
        ...
        return view('admin.menus.form', [
            ...
            'parentOptions' => $this->parentOptions($site->id, $menu->location, $menu->id),
    ...
    public function update(AdminContext $context, Request $request, CmsMenu $menu): RedirectResponse
    {
        ...
        return redirect()->route('admin.menus.edit', $menu)->with('status', "\u{9078}\u{55AE}\u{5DF2}\u{66F4}\u{65B0}");
    }
    ...
    {
        $this->authorizeMenu($context, $menu);
        $redirect = ['location' => $menu->location, 'parent_id' => $menu->parent_id];
    ...
    public function restore(AdminContext $context, int $id): RedirectResponse|JsonResponse
    {
        ...
        $redirectUrl = route('admin.menus.index', array_filter(['location' => $menu->location, 'parent_id' => $menu->parent_id]));
    ...
    public function forceDelete(AdminContext $context, int $id): RedirectResponse|JsonResponse
    {
        ...
        $redirectUrl = route('admin.menus.index', ['location' => $location, 'trash' => 1]);
    ...
    public function bulkAction(AdminContext $context, Request $request): JsonResponse
    {
        ...
        return response()->json([
            ...
            'redirect_url' => $data['action'] === 'restore'
                ? route('admin.menus.index', ['location' => $menus->first()?->location ?? 'backend'])
                : null,
        ]);
    }
    ...
    private function parentOptions(int $siteId, string $location, ?int $excludeId = null): array
    {
        $menus = CmsMenu::query()
    ...
    private function nextSortOrder(int $siteId, string $location, ?int $parentId): int
    {
